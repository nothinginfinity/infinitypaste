// ─── InfinityPaste v4.0.0 — app.js ──────────────────────────────────────────────
// Phase 4: Local Whisper, TF-IDF, distilBART summarizer, compromise.js cleanup, franc lang detection
  ...
  items.forEach((item, idx) => {
    ...
    existing.innerHTML = `
      ...
      <div class="card-actions">
        ...
        ${ item.content.trim().split(/\s+/).length >= 80 ? `<button class="card-btn" id="summarize-btn-${item.id}" onclick="localSummarizeQueueItem(${item.id})" title="Summarize locally (distilBART, ~250MB)">📋</button>` : '' }
        <button class="card-btn" id="cleanup-btn-${item.id}" onclick="cleanupQueueItem(${item.id})" title="Clean up text (compromise.js)">✨</button>
        <button class="card-btn" id="lang-btn-${item.id}" onclick="detectLanguageOfItem(${item.id})" title="Detect language (franc)">🌐</button>
...
// ─── Feature 9: Local Whisper Transcription (Transformers.js) ─────────────────
...
function _loadTransformers() {
  ...
  return new Promise((resolve, reject) => {
    ...
    s.onerror = () => reject(new Error('Failed to load Transformers.js. Check your connection.'));
    document.head.appendChild(s);
  });
}
...
async function _getWhisperPipeline(progressCb) {
  ...
  const { pipeline, env } = window.transformers || await import(TRANSFORMERS_CDN);
...
async function localTranscribeRecording(id) {
  ...
  try {
    ...
    const progressCb = (p) => {
      if (progressEl && p.status === 'downloading') {
        const pct = p.progress ? Math.round(p.progress) : '…';
        progressEl.textContent = `Downloading model: ${pct}%`;
      } else if (progressEl && p.status === 'loading') {
        progressEl.textContent = 'Loading model into memory…';
      }
    };
...
// ─── Feature 10: TF-IDF Keyword Extractor (surfaced as action) ────────────────
...
function _tfidfKeywords(text, count = 10) {
  const stopWords = new Set(['the','a','an','and','or','but','in','on','at','to','for','of','with','by','from','is','was','are','were','be','been','have','has','had','do','does','did','will','would','could','should','may','might','this','that','these','those','i','you','he','she','it','we','they','what','which','who','when','where','why','how','all','each','every','both','few','more','most','other','some','such','no','nor','not','only','own','same','so','than','too','very','just','because','as','until','while','about','into','through','during','before','after','above','below','between','out','off','over','under','again','then','once','here','there','if','can','its','your','our','their','his','her','my','one','two','three','also','get','use','used','using','said','says','like','well','back','even','want','see','know','think','make','made','time','way','new','good','first','last','long','great','little','own','right','big','high','different','small','large','next','early','young','important','public','private','real','best','free','able']);
  ...
  const uniqueTerms = Object.keys(freq).length || 1;

  return Object.entries(freq)
    .map(([word, count]) => ({
      word,
      score: (count / totalWords) * Math.log(1 + uniqueTerms / count),
    }))
    .sort((a, b) => b.score - a.score)
    .slice(0, count);
}

// ─── Feature 11: Local Summarization (distilBART via Transformers.js) ──────────
let _summarizePipeline = null;
let _summarizeLoading  = false;

async function localSummarizeQueueItem(id) {
  const item = queue.find(i => i.id === id);
  if (!item) return;
  const wordCount = item.content.trim().split(/\s+/).length;
  if (wordCount < 80) { showToast('Text too short to summarize (need ~80+ words)', 'error'); return; }
  ...
  try {
    if (!_summarizePipeline) {
      ...
      _summarizePipeline = await pipeline('summarization', 'Xenova/distilbart-cnn-6-6', {
        progress_callback: (p) => {
          if (progressEl && p.status === 'downloading') {
            progressEl.textContent = `Downloading model: ${Math.round(p.progress || 0)}%`;
          }
        },
      });
    ...
    const input = item.content.slice(0, 4000);
    const result = await _summarizePipeline(input, { max_length: 150, min_length: 30 });
...
// ─── Feature 12: Readability Cleanup (compromise.js) ──────────────────────────
...
async function cleanupQueueItem(id) {
  ...
  try {
    ...
    cleaned = cleaned
      .replace(/\bl\b(?=\s+[a-z])/g, 'I')
      .replace(/(?<!\d)0(?=\s+[a-zA-Z])/g, 'O')
      .replace(/\s+([.,!?;:])/g, '$1')
      .replace(/([.,!?;:])(?=[a-zA-Z])/g, '$1 ')
      .replace(/\n[ \t]+/g, '\n')
      .trim();
...
function _loadFranc() {
  ...
  return new Promise((resolve, reject) => {
    import(FRANC_CDN)
      .then(mod => {
        _francDetect = mod.franc || mod.default;
        _francLoaded = true;
        resolve();
      })
      .catch(() => reject(new Error('Failed to load franc. Check your connection.')));
  });
}
...
const ISO_LANG_NAMES = {
  ...
  ara:'Arabic',hin:'Hindi',nld:'Dutch',pol:'Polish',swe:'Swedish',
  nor:'Norwegian',dan:'Danish',fin:'Finnish',tur:'Turkish',vie:'Vietnamese',
  ind:'Indonesian',msa:'Malay',tha:'Thai',ces:'Czech',ron:'Romanian',
  hun:'Hungarian',ukr:'Ukrainian',cat:'Catalan',heb:'Hebrew',
};
...
async function detectLanguageOfItem(id) {
  ...
  try {
    ...
    const unknown = lang === 'und';

    if (unknown) {
      showToast('Language could not be determined (text too short or mixed)');
      return;
    }

    const idx = queue.findIndex(i => i.id === id);
    if (idx >= 0) {
      const existing = queue[idx].label || '';
      if (!existing.includes(name)) {
        queue[idx].label = existing ? `${existing} · ${name}` : name;
        saveQueue();
        renderQueue();
      }
    }
    ...
    queue.forEach((item, idx) => {
      ...
      if (lang !== 'und') {
        ...
        if (!item.label?.includes(name)) {
          queue[idx].label = item.label ? `${item.label} · ${name}` : name;
          tagged++;
        }
      }
    });
